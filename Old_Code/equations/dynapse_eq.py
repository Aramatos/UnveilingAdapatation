...
def dynapse_eq():
    return{'model': '''
                    ...
                    # STD, from Shih Chii Liu's circuit model
                    # dD/dt = (1 - D) / taud : 1
                    dD/dt = M * ( 1 - D**(1/kappa) ) : 1
                    ...
                    Itauahp_clip = Itauahp*(Iahp>Io) + Io*(Iahp<=Io) : amp
                    Ishunt_clip = clip(I_syn_gaba_a, Io, Imem) : amp

                    Iahpmax = (Ica / Itauahp_clip) * Ithahp_clip : amp                # Ratio of currents through diffpair and adaptation block
                    #a = Iahpmax * ahp : amp
                    Ia = Iagain / (1 + exp(-(Imem - Iath) / Ianorm)) : amp       # Positive feedback current

                    tauahp = (Cahp * Ut) / (kappa * Itauahp_clip) : second       # Time constant of adaptation
                    tau = (Cmem * Ut) / (kappa * Itau_clip) : second        # Membrane time constant
                    kappa = (kn + kp) / 2 : 1

                    # constants
                    ahp     : 1     (shared, constant)                 # factor of stength of adaptation. 
                    ...
                    M       : Hz    (constant, shared)                 # Depression rate. Inversely proportional to theoretical depression time constant
                    taud    : second (constant,shared)                 # Depression time constant. 
                    ...
                    Inmdamax = (I_wo_syn_nmda / I_tau_clip_syn_exc) * I_g_clip_syn_exc : amp  #maximum value
                    ...
                    Iampamax = (I_wo_syn_ampa / I_tau_clip_syn_exc2) * I_g_clip_syn_exc2 : amp  #maximum value
                    ...
                    Igababmax = (I_wo_syn_gaba_b / I_tau_clip_syn_inh) * I_g_clip_syn_inh : amp  #maximum value
                    ...
                    Igabaamax = (I_wo_syn_gaba_a / I_tau_clip_syn_shunt) * I_g_clip_syn_shunt : amp  #maximum value
                    ''',
...
def dynapse_nmda_syn_eq(): # SLOW_EXC
    ...
    return{'model': """
                    ...
                    d      : 1 # can only be integer
                    """,
           'on_pre': """
                    D = (D+d)*(D<1) + 1*(D>=1)
                    I_syn_nmda_post +=  ((1-(D*(D<1) + 1*(D>=1)))*I_wo_syn_nmda_post*weight*I_g_syn_nmda_post/(I_tau_syn_nmda_post*((I_g_syn_nmda_post/I_syn_nmda_post)+1)))
                    """,
           'on_post': """ """,
           'method': 'euler'}
...
def dynapse_ampa_syn_eq(): # FAST_EXC
    ...
    return{'model': """
           ...
           'on_pre': """ 
                    D = (D+d)*(D<1) + 1*(D>=1)
                    I_syn_ampa_post +=  ((1-(D*(D<1) + 1*(D>=1)))*I_wo_syn_ampa_post*weight*I_g_syn_ampa_post/(I_tau_syn_ampa_post*((I_g_syn_ampa_post/I_syn_ampa_post)+1)))
                    """,
           'on_post': """ """,
           'method': 'euler'}
