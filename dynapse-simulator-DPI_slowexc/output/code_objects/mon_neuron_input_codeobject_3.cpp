#include "code_objects/mon_neuron_input_codeobject_3.h"
#include "objects.h"
...
void _run_mon_neuron_input_codeobject_3()
{
...
const size_t _num_source_idx = 1;
    ...
    int32_t*   _ptr_array_SpikeGenerator_i = _array_SpikeGenerator_i;
    ...
    int32_t _num_events = _ptr_array_SpikeGenerator__spikespace[_num_spikespace-1];
    ...
    if (_num_events > 0)
    {
        ...
        {
            const int _idx = _ptr_array_SpikeGenerator__spikespace[_j];
        ...
        if (_num_events > 0) {
            ...
            {
                const size_t _idx = _ptr_array_SpikeGenerator__spikespace[_j];
                const size_t _vectorisation_idx = _idx;
                                
                const int32_t _source_i = _ptr_array_SpikeGenerator_i[_idx];
                const double _to_record_t = _source_t;
                const int32_t _to_record_i = _source_i;

                _dynamic_array_mon_neuron_input_i.push_back(_to_record_i);
                _dynamic_array_mon_neuron_input_t.push_back(_to_record_t);
                _ptr_array_mon_neuron_input_count[_idx-_source_start]++;
            }
            _ptr_array_mon_neuron_input_N[0] += _num_events;
        }
    }
...
void _debugmsg_mon_neuron_input_codeobject_3()
{
    using namespace brian;
    const size_t _numN = 1;
const size_t _numcount = 1;
