...
def dynapse_eq():
    return{'model': '''
                    ...
                    Isoma_sum = Isoma_dpi_tau_shunt + Igaba_a_shunt - Isoma_pfb_shunt - I0*(Isoma_mem<=I0) : amp  # Auxiliary variable
                    ...
                    Iin_clip = clip(Inmda_dp + Iampa + Iampa_std - Igaba_b + Isoma_const - Isoma_ahp_shunt, I0, 1*amp) : amp
                    ...
                    # AMPA_std ######################################################
                    dIampa_std/dt = (- Iampa_std - Iampa_std_g_shunt + 2*I0*(Iampa_std <= I0))/(tau_ampa_std * ((Iampa_std_g_shunt / Iampa_std) + 1)) : amp
                    
                    Iampa_std_tau_shunt = Iampa_tau*(Iampa_std>I0) + I0*(Iampa_std<=I0) : amp       # Shunt tau current if Iampa goes to I0
                    Iampa_std_g_shunt = Iampa_std_g*(Iampa_std>I0) + I0*(Iampa_std<=I0) : amp       # Shunt g current if Iampa goes to I
                   

                    #Vw_shunt = Va*(Vw>Va) + Vw*(Vw<Va) : volt 
                    Iampa_std_w = I0*exp(kappa*Vw/Ut) : amp                                         # Weight transistor
                    dVw/dt = (Ia-(std_pulse<.9)*Idischarge_w)/Campa_std : volt                                           #Capacitor current and Vw
                    Ia=I0*exp(kappa*(Va-Vw)/Ut)*(1-exp((Vw-Va)/Ut)) : amp                 #Diode connected transistor

                    dstd_pulse/dt=(1-std_pulse)/tau_pulse:1 

                    tau_pulse: second (constant)
                    Idischarge_w: amp (constant) 
                    Va : volt (constant)                                                   # voltage of diode connected transistor in STD circuit 
                    Campa_std : farad (constant)                                          # Synapse's capacitance
                    Iampa_std_tau : amp (constant)                                          # Leakage current, i.e. how much current is constantly leaked away (time-constant)
                    Iampa_std_g = alpha_ampa_std * Iampa_std_tau : amp                              # AMPA synapse gain expressed in terms of its tau current
                    tau_ampa_std = Campa_std * Ut / (kappa * Iampa_std_tau_shunt) : second          # Synaptic time-constant
...
def dynapse_ampa_std_syn_eq():  # FAST_EXC
    """This function returns the fast excitatory synapse equation dictionary.
    """
    return{'model': """
                    weight : 1 # Can only be integer on the chip
