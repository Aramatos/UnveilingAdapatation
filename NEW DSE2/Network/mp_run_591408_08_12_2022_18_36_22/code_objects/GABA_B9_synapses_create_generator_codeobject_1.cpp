...
void _run_GABA_B9_synapses_create_generator_codeobject_1()
{
...
const size_t _numN_incoming = _dynamic_array_GABA_B9_N_incoming.size();
    ...
    _dynamic_array_GABA_B9_N_incoming.resize(_N_post + _target_offset);
    _dynamic_array_GABA_B9_N_outgoing.resize(_N_pre + _source_offset);
    ...
    for(size_t _i=0; _i<_N_pre; _i++)
    {
        ...
        for(long _k=_uiter_low; _k<_uiter_high; _k+=_uiter_step)
        {
            ...
            for (size_t _repetition=0; _repetition<_n; _repetition++) {
                _dynamic_array_GABA_B9_N_outgoing[_pre_idx] += 1;
                _dynamic_array_GABA_B9_N_incoming[_post_idx] += 1;
                _dynamic_array_GABA_B9__synaptic_pre.push_back(_pre_idx);
                _dynamic_array_GABA_B9__synaptic_post.push_back(_post_idx);
			}
		}
	}

	// now we need to resize all registered variables
	const int32_t newsize = _dynamic_array_GABA_B9__synaptic_pre.size();
    _dynamic_array_GABA_B9__synaptic_post.resize(newsize);
    _dynamic_array_GABA_B9__synaptic_pre.resize(newsize);
    _dynamic_array_GABA_B9_delay.resize(newsize);
    _dynamic_array_GABA_B9_delay_1.resize(newsize);
    _dynamic_array_GABA_B9_weight.resize(newsize);
	// Also update the total number of synapses
	_ptr_array_GABA_B9_N[0] = newsize;
