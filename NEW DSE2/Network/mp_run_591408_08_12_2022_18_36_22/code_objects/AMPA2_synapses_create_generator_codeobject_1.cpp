...
int32_t* const _array_AMPA2_N_outgoing = _dynamic_array_AMPA2_N_outgoing.empty()? 0 : &_dynamic_array_AMPA2_N_outgoing[0];
const size_t _numN_outgoing = _dynamic_array_AMPA2_N_outgoing.size();
...
const size_t _num_synaptic_post = _dynamic_array_AMPA2__synaptic_post.size();
const size_t _numN = 1;
int32_t* const _array_AMPA2_N_incoming = _dynamic_array_AMPA2_N_incoming.empty()? 0 : &_dynamic_array_AMPA2_N_incoming[0];
const size_t _numN_incoming = _dynamic_array_AMPA2_N_incoming.size();
    ///// POINTERS ////////////
        
    int32_t* __restrict  _ptr_array_AMPA2__synaptic_pre = _array_AMPA2__synaptic_pre;
    int32_t* __restrict  _ptr_array_AMPA2_N_outgoing = _array_AMPA2_N_outgoing;
    int32_t* __restrict  _ptr_array_AMPA2__synaptic_post = _array_AMPA2__synaptic_post;
    int32_t*   _ptr_array_AMPA2_N = _array_AMPA2_N;
    int32_t* __restrict  _ptr_array_AMPA2_N_incoming = _array_AMPA2_N_incoming;
    ...
    _dynamic_array_AMPA2_N_incoming.resize(_N_post + _target_offset);
    _dynamic_array_AMPA2_N_outgoing.resize(_N_pre + _source_offset);
    ...
    for(size_t _i=0; _i<_N_pre; _i++)
    {
        ...
        for(long _k=_uiter_low; _k<_uiter_high; _k+=_uiter_step)
        {
            ...
            for (size_t _repetition=0; _repetition<_n; _repetition++) {
                _dynamic_array_AMPA2_N_outgoing[_pre_idx] += 1;
                _dynamic_array_AMPA2_N_incoming[_post_idx] += 1;
                _dynamic_array_AMPA2__synaptic_pre.push_back(_pre_idx);
                _dynamic_array_AMPA2__synaptic_post.push_back(_post_idx);
			}
		}
	}

	// now we need to resize all registered variables
	const int32_t newsize = _dynamic_array_AMPA2__synaptic_pre.size();
    _dynamic_array_AMPA2__synaptic_post.resize(newsize);
    _dynamic_array_AMPA2__synaptic_pre.resize(newsize);
    _dynamic_array_AMPA2_delay.resize(newsize);
    _dynamic_array_AMPA2_delay_1.resize(newsize);
    _dynamic_array_AMPA2_weight.resize(newsize);
	// Also update the total number of synapses
	_ptr_array_AMPA2_N[0] = newsize;
